...
def _ensure_preexisting(ctx):
    use_existing = ctx.node.properties['use_existing']
    if not use_existing:
        raise NonRecoverableError('Can currently only handle pre-existing NSO')
...
def add_device(ctx, device_ip):
    ...
    if ctx.operation.retry_number == 0:
        ...
        response = requests.patch('{}/api/running/devices/device'.format(nso_url), json=add_payload, headers=STANDARD_HEADERS,
                                  auth=nso_auth)
    ...
    response = requests.post('{0}/api/operations/devices/device/{1}/ssh/fetch-host-keys'.format(
        nso_url, device_name),
        auth=nso_auth)
    ...
    try:
        ...
        if '<result>failed</result>' in response.text:
            return ctx.operation.retry(message="Received failure status: {0}".format(response.text))
    except HTTPError as ex:
        return ctx.operation.retry(message="Received HTTP error: {0}".format(str(ex)))
...
def _get_nso_device_url(node, datastore, device_name):
    return _get_nso_url(node, datastore, "/devices/device/{0}".format(device_name))
...
def _sync_from(ctx, nso_node):
    ...
    response = requests.post('{0}/api/running/devices/_operations/sync-from'.format(nso_url),
                             headers=STANDARD_HEADERS,
...
def _test_response(ctx, response, desc):
    try:
        ...
        _, _, tb = sys.exc_info()
        raise NonRecoverableError('Failed during {0}'.format(desc),
